...
transporter.verify((error, success) => {
    if (error) {
        ...
        console.log('⚠️ Les emails ne seront pas envoyés. Vérifiez votre .env');
    } else {
...
<body>
    <div class="container">
        ...
        <div class="footer">
            ...
            <p>📧 contact@en-bus.com | 📱 +242 06 123 4567</p>
...
Heure de départ : ${reservation.route.departure}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Vous devez payer avant le ${new Date(reservation.paymentDeadline).toLocaleString('fr-FR')}
...
• Bagages : 1 bagage en soute (20kg) + 1 bagage à main inclus
${reservation.route.trackerId ? `• Code de suivi GPS : ${reservation.route.trackerId}` : ''}

Bon voyage avec En-Bus !

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
En-Bus - Voyagez à travers l'Afrique
📧 contact@en-bus.com | 📱 +242 06 123 4567
    `;
...
// ============================================
// ✅ FONCTION EMAIL DE RAPPEL 24H AVANT
// ============================================
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>En-Bus - contact@en-bus.com - +242 06 123 4567</p>
        </div>
    </div>
</body>
</html>
...
// ============================================
// ✅ FONCTION EMAIL PAIEMENT EXPIRÉ
// ============================================
...
Pour voyager, vous devrez faire une nouvelle réservation sur notre site.
...
Nous avons bien reçu votre paiement pour la réservation ${reservation.bookingNumber}.
...
app.get('/', (req, res) => {
    res.send(`
        <h1>🚌 Serveur En-Bus Backend</h1>
        <p>Le serveur est en marche et fonctionnel.</p>
        <h2>📧 Service email : ${transporter ? '✅ Activé' : '❌ Désactivé'}</h2>
        <h2>Routes API disponibles :</h2>
        <ul>
            <li>POST /api/reservations - Créer une réservation (+ email auto)</li>
            <li>GET /api/reservations/user/:phone - Réservations par téléphone</li>
            <li>GET /api/reservations/:bookingNumber - Détails d'une réservation</li>
            <li>PATCH /api/reservations/:bookingNumber/cancel - Annuler une réservation</li>
            <li>PATCH /api/reservations/:bookingNumber/confirm-payment - Confirmer paiement agence (+ email)</li>
            <li>GET /api/admin/reservations - Liste complète (admin)</li>
            <li>POST /track/update - Mise à jour GPS</li>
        </ul>
    `);
});
...
dbClient.on('close', () => {
    console.log("⚠️ Connexion MongoDB fermée, tentative de reconnexion...");
    connectToDb();
});

// ============================================
// ROUTES POSITIONS GPS
// ============================================
app.post('/track/update', async (req, res) => {
    ...
    console.log(`🛰️  [HTTP] Position reçue pour le bus ${busId}`);
    ...
    try {
        ...
        const clientsInRoom = io.sockets.adapter.rooms.get(busId);
        console.log(`📡  [WEBSOCKET] Position diffusée à '${busId}'. Clients: ${clientsInRoom ? clientsInRoom.size : 0}`);
...
// ✅ Créer une réservation (+ ENVOI EMAIL AUTOMATIQUE) - CORRIGÉ
app.post('/api/reservations', async (req, res) => {
    try {
        const reservation = req.body;
        
        console.log("📥 Réservation reçue:", reservation.bookingNumber);
        
        // Validation de base
        if (!reservation.bookingNumber) {
            return res.status(400).json({ error: 'Numéro de réservation manquant' });
        }
        
        if (!reservation.passengers || reservation.passengers.length === 0) {
            return res.status(400).json({ error: 'Informations passagers manquantes' });
        }
        
        // ✅ CORRECTION : Validation paiement agence (1h minimum au lieu de 48h)
        if (reservation.paymentMethod === 'agency') {
            ...
            console.log(`⏰ Paiement agence - Heures avant départ: ${hoursUntilDeparture.toFixed(1)}h`);
            
            // ✅ MODIFIÉ : 1h minimum au lieu de 48h
            if (hoursUntilDeparture < 1) {
                return res.status(400).json({ 
                    error: 'Paiement en agence non disponible (moins de 1h avant le départ)' 
                });
            }
            
            if (!reservation.paymentDeadline) {
                return res.status(400).json({ 
                    error: 'Date limite de paiement manquante' 
                });
            }
            
            console.log(`✅ Paiement agence autorisé - Deadline: ${reservation.paymentDeadline}`);
        }
        ...
        if (existing) {
            return res.status(409).json({ error: 'Ce numéro de réservation existe déjà' });
        }
        ...
        // ✅ ENVOYER EMAIL DE CONFIRMATION (en arrière-plan, ne bloque pas la réponse)
        if (reservation.passengers[0]?.email) {
            sendConfirmationEmail(reservation)
                .then(result => {
                    if (result.success) {
                        console.log(`📧 Email confirmation envoyé à ${result.recipient}`);
                    } else {
                        console.log(`⚠️ Email non envoyé : ${result.reason || result.error}`);
                    }
                })
                .catch(err => {
                    console.error('❌ Erreur envoi email (non bloquant):', err.message);
                });
        } else {
            console.log('⚠️ Aucun email fourni, email non envoyé');
        }
...
app.get('/api/reservations/user/:phone', async (req, res) => {
    try {
        ...
        console.log(`📋 Recherche réservations pour: ${phone}`);
...
app.get('/api/reservations/:bookingNumber', async (req, res) => {
    try {
        ...
        console.log(`🔍 Recherche réservation: ${bookingNumber}`);
        ...
        console.log(`✅ Réservation trouvée: ${bookingNumber}`);
...
app.patch('/api/reservations/:bookingNumber/cancel', async (req, res) => {
    try {
        const bookingNumber = req.params.bookingNumber;
        
        console.log(`🚫 Annulation réservation: ${bookingNumber}`);
...
// ✅ Confirmer le paiement en agence (+ ENVOI EMAIL)
app.patch('/api/reservations/:bookingNumber/confirm-payment', async (req, res) => {
    try {
        const bookingNumber = req.params.bookingNumber;
        
        console.log(`💰 Confirmation paiement agence: ${bookingNumber}`);
        ...
        console.log(`✅ Paiement confirmé: ${bookingNumber}`);
        
        if (reservation.passengers[0]?.email) {
            sendPaymentConfirmedEmail(reservation)
                .then(result => {
                    if (result.success) {
                        console.log(`📧 Email confirmation paiement envoyé à ${result.recipient || reservation.passengers[0].email}`);
                    }
                })
                .catch(err => {
                    console.error('❌ Erreur envoi email confirmation paiement:', err.message);
                });
        }
...
// ============================================
// ✅ CRON JOB : ANNULATION + RAPPELS
// ============================================

// Vérifier toutes les heures les réservations expirées
cron.schedule('0 * * * *', async () => {
    ...
    try {
        ...
        if (expiredReservations.length === 0) {
            console.log('✅ [CRON] Aucune réservation expirée');
        } else {
                ...
                await reservationsCollection.updateOne(
                    ...
                    { 
                        $set: { 
                            status: 'Expiré',
                            cancelledAt: now,
                            cancelReason: 'Paiement non effectué dans les 10h',
                            updatedAt: now
                        } 
                    }
                );
                
                console.log(`❌ [CRON] ${reservation.bookingNumber} → Expiré (deadline: ${reservation.paymentDeadline})`);
                
                if (reservation.passengers[0]?.email) {
                    sendExpirationEmail(reservation).catch(err => {
                        console.error('Erreur email expiration:', err.message);
                    });
                }
            }
...
// ✅ Rappel 24h avant le départ (tous les jours à 8h)
cron.schedule('0 8 * * *', async () => {
    console.log('📧 [CRON] Envoi des rappels 24h...');
...
console.log('   - Vérification paiements agence : toutes les heures');
console.log('   - Rappels 24h : tous les jours à 8h');

// ============================================
// WEBSOCKET (SUIVI GPS TEMPS RÉEL)
// ============================================
io.on('connection', (socket) => {
    ...
    socket.on('subscribeToBus', async (busId, callback) => {
        ...
        console.log(`↪️ [WEBSOCKET] Client ${socket.id} s'abonne à '${busId}'`);
        socket.join(busId);
        
        try {
            const lastPosition = await positionsCollection.findOne({ busId: busId });
            
            if (lastPosition) {
                console.log(`📍 Position trouvée pour '${busId}'. Envoi au client.`);
                socket.emit('updatePosition', lastPosition);
                if(callback) callback({ status: "ok", message: "Abonné avec dernière position." });
            } else {
                console.log(`❓ Aucune position pour '${busId}'. En attente.`);
                if(callback) callback({ status: "ok", message: "Abonné, aucune position initiale." });
            }
        } catch (error) { 
...
📧 Service email : ${transporter ? '✅ Activé (Nodemailer)' : '❌ Désactivé'}
⏰ Cron jobs : ✅ Activés
✅ Paiement agence : minimum 1h avant départ

📡 Routes API :
   POST   /api/reservations (+ email auto)
   GET    /api/reservations/user/:phone
   GET    /api/reservations/:bookingNumber
   PATCH  /api/reservations/:bookingNumber/cancel
   PATCH  /api/reservations/:bookingNumber/confirm-payment (+ email)
